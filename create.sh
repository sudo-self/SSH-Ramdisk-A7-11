...
case $device in
  iPhone6,1 | iPhone6,2 | iPad4,1 | iPad4,2 | iPad4,3 | iPad4,4 | iPad4,5 | iPad4,6 | iPad4,7 | iPad4,8 | iPad4,9) # iPhone 5s / iPad Air / iPad Mini 2 & 3
    chip="A7"
    cpid="0x8960";;
  iPhone7,1 | iPhone7,2 | iPad5,1 | iPad5,2 | iPod7,1) # iPhone 6 / 6 Plus / iPad Mini 4 / iPod 6
    chip="A8"
    cpid="0x7000";;
  iPad5,3 | iPad5,4) # iPad Air 2 (A8X)
    chip="A8"
    cpid="0x7001";;
  iPhone8,1 | iPhone8,2 | iPhone8,4 | iPad6,11 | iPad6,12) # iPhone 6s / 6s Plus / SE1 / iPad 5
    chip="A9";; # Use S8000/S8003 chip selection below
  iPad6,3 | iPad6,4 | iPad6,7 | iPad6,8) # A9X
    chip="A9"
    cpid="0x8001";;
  iPhone9,1 | iPhone9,2 | iPhone9,3 | iPhone9,4 | iPad7,5 | iPad7,6 | iPad7,11 | iPad7,12 | iPod9,1) # iPhone 7 / 7 Plus / iPad 6 / iPad 7 / iPod 7
    chip="A10"
    cpid="0x8010";;
  iPad7,1 | iPad7,2 | iPad7,3 | iPad7,4) # A10X
    chip="A10"
    cpid="0x8011";;
...
# Get the board name for the device type
case $device in
  iPhone6,1)
    model=n51ap;;
  iPhone6,2)
...
if ! [ -z "$3" ] && [ "$3" != "-i" ] && [ "$3" != "-t" ] && [ "$3" != "-b" ]; then
  ...
  build=$3
  extra=$4
  curl -s -o /tmp/firmwarekeys.txt "https://${wikiprefix}the${wiki}wiki.com/wiki/$keypath$codename"_"$build"_"($device)?action=raw"
  ...
  codename="$((curl -s "https://${wikiprefix}the${wiki}wiki.com/wiki/Firmware_Keys/$majorversion.x") | grep "$BuildID"_"" |  grep $device -m 1| awk -F_ '{print $1}' | awk -F"wiki" '{print "wiki"$2}')"
  ...
  if ! [ -d .decrypted_$device ] && ! [ -e *-$device/build/decrypted/files_decrypted ]; then
    ...
    curl -s -o /tmp/firmwarekeys.txt "https://${wikiprefix}the${wiki}wiki.com/$codename"_"$BuildID"_"($device)?action=raw"
    if ! [ -e "/tmp/firmwarekeys.txt" ]; then
    ...
    if grep -q "404 Not Found" /tmp/firmwarekeys.txt; then
      ...
      echo "To fix this error, go to https://${wikiprefix}the${wiki}wiki.com/wiki/Firmware_Keys/$majorversion.x and choose the closest iOS version for your device that has a blue link."
...
echo "Creating $buildtype for device $device ($chip) with base version $version"
...
if [ "$device" == "iPhone8,1" ] || [ "$device" == "iPhone8,2" ] || [ "$device" == "iPad6,11" ] || [ "$device" == "iPad6,12" ]; then # iPhone 6s / 6s Plus / iPad 5
  ...
  echo "Downloading files for chip $chipname for A9 device: $device"
...
download_manifest() {
   if ! [ -e BuildManifest.plist ]; then
    if [ -z "$model" ]; then
      echo "ERROR: Cannot find device board name"
      exit 1
    fi
    echo "Downloading build manifest..."
    DYLD_LIBRARY_PATH=../../resources/bin/.libs ../../resources/bin/pzb -g BuildManifest.plist "$ipsw_link" > /dev/null
    if ! [ -e BuildManifest.plist ]; then
      echo "Error occurred while downloading build manifest ($PWD)"
      exit 1
    fi
  fi
}
...
for i in $range; do
  ...
  if [ -z "$component" ] || [ "$component" == ".dmg" ]; then
    # January 2024: Filenames for unencrypted files removed from wiki, find filenames from build manifest instead
    download_manifest
    #extralog="(from build manifest)"
    echo "No filename for $filetype in key page, using build manifest instead"
    case $filetype in
      iBSS | iBEC | iBoot)
        componentpath=""
        component="$(awk "/""${model}""/{x=1}x&&/${filetype}[.]/{print;exit}" BuildManifest.plist | grep '<string>' |cut -d\> -f2 |cut -d\< -f1)";;
      DeviceTree)
        component="DeviceTree.$model.im4p";;
      kernelcache)
        componentpath=""
        component="$(awk "/""${model}""/{x=1}x&&/kernelcache.release/{print;exit}" BuildManifest.plist | grep '<string>' |cut -d\> -f2 |cut -d\< -f1)";;
      RestoreRamDisk)
        component="$(/usr/bin/plutil -extract "BuildIdentities".0."Manifest"."RestoreRamDisk"."Info"."Path" xml1 -o - BuildManifest.plist | grep '<string>' |cut -d\> -f2 |cut -d\< -f1 | head -1)";;
      UpdateRamDisk)
        for i in {0..12}; do if plutil -extract BuildIdentities.$i xml1 -o - BuildManifest.plist | plutil -p - | grep Variant | grep -q "Upgrade Install"; then echo "Found update build type at $i"; buildindex=$i; break; fi; done
        component="$(/usr/bin/plutil -extract "BuildIdentities".${buildindex}."Manifest"."RestoreRamDisk"."Info"."Path" xml1 -o - BuildManifest.plist | grep '<string>' |cut -d\> -f2 |cut -d\< -f1 | head -1)";;
    esac
  fi
  ...
  echo "Downloading ${filetype}${altfile} ($component)...$extralog"
...
if ! bootcheck && [ "$majorversion" -gt "11" ]; then
  ...
  if [ -z "$rootfsname" ]; then
    component="$(/usr/bin/plutil -extract "BuildIdentities".0."Manifest"."OS"."Info"."Path" xml1 -o - BuildManifest.plist | grep '<string>' |cut -d\> -f2 |cut -d\< -f1 | head -1)"
  fi

  if [ -z "$rootfsname" ]; then
    echo "ERROR: Cannot find filename for root filesystem!"
    exit 1
  fi
...
if ! a10check && ! [ -f decrypted/iBSS.dec ]; then echo "ERROR: Failed to download iBSS!"; exit 1; fi
if ! [ -f decrypted/iBEC.dec ]; then echo "ERROR: Failed to download iBEC!"; exit 1; fi
if ! [ -f decrypted/DeviceTree.dec ]; then echo "ERROR: Failed to download device tree!"; exit 1; fi
if ! [ -f decrypted/kernelcache.dec ]; then echo "ERROR: Failed to download kernelcache!"; exit 1; fi
if bootcheck && ! [ -f decrypted/ramdisk.dec.dmg ]; then echo "ERROR: Failed to download ramdisk!"; exit 1; fi
...
../../resources/bin/sed -i 's/RELEASE_ARM/MeowRamdisk/g' ./patched/kernelcache.patched
...
sudo plutil -convert xml1 ./mnt/System/Library/LaunchDaemons/com.apple.restored_${daemon}.plist
sudo ../../resources/bin/sed -i 's|/restored_\(.*\)</string>|/restored_\1</string><string>-server</string>|' ./mnt/System/Library/LaunchDaemons/com.apple.restored_${daemon}.plist
...
if [ "$majorversion" -gt 13 ]; then
  sudo mkdir ./mnt/tmpfs
fi
sudo ln -s /mnt2/fbin ./mnt/var/fbin
