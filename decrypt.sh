...
# This file is part of the 64-bit ramdisk tool by meowcat454 (version 0.18)
set -e
...
DYLD_LIBRARY_PATH=../../resources/bin/.libs ../resources/bin/pzb -g "$(awk "/""$cpid""/{x=1}x&&/iBSS[.]/{print;exit}" BuildManifest.plist | grep '<string>' | cut -d\> -f2 | cut -d\< -f1)" "$ipsw_link" > /dev/null
...
DYLD_LIBRARY_PATH=../../resources/bin/.libs ../resources/bin/pzb -g "$(awk "/""$cpid""/{x=1}x&&/iBEC[.]/{print;exit}" BuildManifest.plist | grep '<string>' | cut -d\> -f2 | cut -d\< -f1)" "$ipsw_link" > /dev/null
...
DYLD_LIBRARY_PATH=../../resources/bin/.libs ../resources/bin/pzb -g Firmware/all_flash/DeviceTree."$model".im4p "$ipsw_link" > /dev/null
../resources/bin/img4 -i DeviceTree."$model".im4p -o DeviceTree.dec > /dev/null

if [ "$3" == "-b" ]; then
  echo "[*] Downloading trustcache"
  DYLD_LIBRARY_PATH=../../resources/bin/.libs ../resources/bin/pzb -g "$(/usr/bin/plutil -extract "BuildIdentities".0."Manifest"."StaticTrustCache"."Info"."Path" xml1 -o - BuildManifest.plist | grep '<string>' | cut -d\> -f2 | cut -d\< -f1 | head -1)" "$ipsw_link" > /dev/null
else
  echo "[*] Downloading ramdisk"
  rdpath="$(/usr/bin/plutil -extract "BuildIdentities".0."Manifest"."RestoreRamDisk"."Info"."Path" xml1 -o - BuildManifest.plist | grep '<string>' | cut -d\> -f2 | cut -d\< -f1 | head -1)"
  DYLD_LIBRARY_PATH=../../resources/bin/.libs ../resources/bin/pzb -g "$rdpath" $ipsw_link > /dev/null
  ../resources/bin/img4 -i *.dmg -o ramdisk.dec.dmg ${iv}${key} > /dev/null
  echo "[*] Downloading trustcache"
  DYLD_LIBRARY_PATH=../../resources/bin/.libs ../resources/bin/pzb -g Firmware/$rdpath.trustcache "$ipsw_link" > /dev/null || true
fi
echo "[*] Downloading kernelcache"
DYLD_LIBRARY_PATH=../../resources/bin/.libs ../resources/bin/pzb -g "$(awk "/""$cpid""/{x=1}x&&/kernelcache.release/{print;exit}" BuildManifest.plist | grep '<string>' | cut -d\> -f2 | cut -d\< -f1)" "$ipsw_link" > /dev/null
